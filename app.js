...
(function () {
  ...
  function getClickMultiplier() {
    return isBuffActive() ? CONFIG.BUFF_MULTIPLIER : 1;
  }

  function activateBuff(durationMs) {
    const until = Date.now() + durationMs;
    state.buffUntil = Math.max(state.buffUntil || 0, until);
    persist();
    renderBuffBanner();
  }
  ...
  function startAutoBuffTimer() {
    setInterval(() => {
      activateBuff(CONFIG.AUTO_BUFF_DURATION_MS);
  ...
  // ---------- ВСПЛЫВАЮЩЕЕ ПРЕДЛОЖЕНИЕ РЕКЛАМЫ (буст) ----------
  function scheduleAdOffer() {
    const min = CONFIG.AD_OFFER_MIN_INTERVAL_MS, max = CONFIG.AD_OFFER_MAX_INTERVAL_MS;
    const delay = min + Math.random() * (max - min);
    adOfferTimer = setTimeout(showAdOffer, delay);
  }
  ...
  async function claimAdOfferBuff() {
    hideAdOffer();
    if (!CONFIG.ADSGRAM_REWARD_BLOCK_ID || !adsgramReward) {
      activateBuff(CONFIG.AD_BUFF_DURATION_MS);
      toast(`⚡ x${CONFIG.BUFF_MULTIPLIER} к клику на ${CONFIG.AD_BUFF_DURATION_MS / 1000} сек!`);
      return;
    }
    try {
  ...
  function bindEvents() {
    ...
    $("#adOfferWatch").addEventListener("click", claimAdOfferBuff);
